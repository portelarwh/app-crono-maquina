...
function updateLapQty(lapId,value){const l=state.laps.find(x=>x.id===lapId);if(!l)return;const v=parseFloat(value);l.qty=Number.isFinite(v)&&v>=0?v:null;render();persist()}
...
function changeTimeUnit(){syncTargets('target');render();persist()}function changeAnalysisMode(){render();persist()}
function closeQtyModal(ok){if(ok&&state.pendingLap){const q=n(els.qtyModalInput,0);addLap(state.pendingLap.durationMs,q,state.pendingLap.endedAt)}state.pendingLap=null;els.qtyModal.style.display='none'}
...
function exportToExcel(){const rows=[['Amostra','Tempo_s','Quantidade','Observacao'],...state.laps.map((l,i)=>[i+1,(l.durationMs/1000).toFixed(2),qty(l),l.obs])];const csv=rows.map(r=>r.map(v=>`"${String(v).replaceAll('"','""')}"`).join(';')).join('\n');downloadBlob(csv,'crono-maquina.csv','text/csv;charset=utf-8')}
