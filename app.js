...
function oeeSet(id,value){const el=$(id);if(el)el.textContent=value}
function oeeVal(id,fb=0){return n($(id),fb)}
function oeePct(v){return Number.isFinite(v)?`${(v*100).toFixed(1)}%`:'--'}
function oeeQty(v){return Number.isFinite(v)?v.toFixed(1).replace('.',','):'—'}
function oeeTime(ms){const s=Math.max(0,Math.floor((ms||0)/1000));const h=Math.floor(s/3600),m=Math.floor((s%3600)/60),ss=s%60;return h?`${h}h ${String(m).padStart(2,'0')}m`:`${String(m).padStart(2,'0')}:${String(ss).padStart(2,'0')}`}
function sanitizeOee(o){if(!o||typeof o!=='object')return null;const num=k=>Number.isFinite(Number(o[k]))?Number(o[k]):NaN;const out={oee:num('oee'),availability:num('availability'),performance:num('performance'),quality:num('quality'),actualQty:num('actualQty'),expectedQty:num('expectedQty'),scrapQty:num('scrapQty'),reworkQty:num('reworkQty'),plannedMs:num('plannedMs'),downtimeMs:num('downtimeMs'),runMs:num('runMs'),configuredRate:num('configuredRate'),unitLabel:typeof o.unitLabel==='string'?o.unitLabel.slice(0,20):'un/h',mainLoss:typeof o.mainLoss==='string'?o.mainLoss.slice(0,40):'',diagnosis:typeof o.diagnosis==='string'?o.diagnosis.slice(0,220):'',savedAt:Number(o.savedAt)||Date.now()};return Number.isFinite(out.oee)?out:null}
function oeeBase(){const s=stats(),plannedMs=totalMs(),downtimeMs=downtimesOf().reduce((a,d)=>a+(d.durationMs||0),0)+activeDowntimeDurationMs(),runMs=Math.max(0,plannedMs-downtimeMs),base=n(els.timeUnit,3600),unitLabel=els.timeUnit.value==='60'?'un/min':'un/h',target=n(els.target,0),takt=n(els.takt,0),units=n(els.units,1)||1;let ratePerSec=target>0?target/base:0;if(!ratePerSec&&takt>0)ratePerSec=units/takt;const configuredRate=ratePerSec*base,expectedQty=ratePerSec*(runMs/1000);return{stats:s,plannedMs,downtimeMs,runMs,base,unitLabel,configuredRate,expectedQty}}
function renderOeeCalc(){const b=oeeBase(),actual=oeeVal('oeeActualQty',0),scrap=Math.max(0,oeeVal('oeeScrapQty',0)),rework=Math.max(0,oeeVal('oeeReworkQty',0)),reworkLoss=$('oeeReworkAsLoss')?.checked?rework:0,good=Math.max(0,actual-scrap-reworkLoss),availability=b.plannedMs>0?Math.min(1,b.runMs/b.plannedMs):NaN,performance=b.expectedQty>0?Math.min(1,actual/b.expectedQty):NaN,quality=actual>0?Math.min(1,good/actual):NaN,oee=[availability,performance,quality].every(Number.isFinite)?availability*performance*quality:NaN;oeeSet('oeeTotalTime',oeeTime(b.plannedMs));oeeSet('oeeDowntimeTime',oeeTime(b.downtimeMs));oeeSet('oeeRunTime',oeeTime(b.runMs));oeeSet('oeeConfiguredSpeed',b.configuredRate>0?`${oeeQty(b.configuredRate)} ${b.unitLabel}`:'—');oeeSet('oeeExpectedQty',b.expectedQty>0?`${oeeQty(b.expectedQty)} un`:'—');oeeSet('oeeValue',oeePct(oee));oeeSet('oeeAvailability',oeePct(availability));oeeSet('oeePerformance',oeePct(performance));oeeSet('oeeQuality',oeePct(quality));const losses=[{name:'Disponibilidade',value:Number.isFinite(availability)?1-availability:-1},{name:'Performance',value:Number.isFinite(performance)?1-performance:-1},{name:'Qualidade',value:Number.isFinite(quality)?1-quality:-1}].sort((a,b)=>b.value-a.value),main=losses[0]&&losses[0].value>0.001?losses[0].name:'Sem perda dominante',perfNote=actual<b.expectedQty?' A produção real ficou abaixo da esperada para o tempo rodando, indicando queda de performance.':'',diag=$('oeeDiagnosis');let diagnosis=`Principal perda: ${main}. OEE calculado como Disponibilidade × Performance × Qualidade.${perfNote}`;if(!b.plannedMs)diagnosis='Registre tempo de medição antes de calcular o OEE.';else if(!b.configuredRate)diagnosis='Informe Meta ou Takt Time para estimar a produção esperada e calcular Performance.';else if(!actual)diagnosis='Informe a quantidade real produzida. Se ficar menor que a esperada com máquina rodando, a perda aparecerá em Performance.';if(diag)diag.textContent=diagnosis;return{oee,availability,performance,quality,actualQty:actual,expectedQty:b.expectedQty,scrapQty:scrap,reworkQty:rework,plannedMs:b.plannedMs,downtimeMs:b.downtimeMs,runMs:b.runMs,configuredRate:b.configuredRate,unitLabel:b.unitLabel,mainLoss:main,diagnosis}}
function openOee(){const modal=els.oeeModal;if(!modal)return;const b=oeeBase(),saved=state.oee;$('oeeActualQty').value=Number.isFinite(saved?.actualQty)?saved.actualQty:(b.stats.q?Number(b.stats.q).toFixed(2):'');$('oeeScrapQty').value=Number.isFinite(saved?.scrapQty)?saved.scrapQty:'0';$('oeeReworkQty').value=Number.isFinite(saved?.reworkQty)?saved.reworkQty:'0';if($('oeeReworkAsLoss'))$('oeeReworkAsLoss').checked=true;modal.style.display='flex';renderOeeCalc();setTimeout(()=>$('oeeActualQty')?.focus(),50)}
function closeOee(){if(els.oeeModal)els.oeeModal.style.display='none'}
function saveOee(){const o=sanitizeOee({...renderOeeCalc(),savedAt:Date.now()});if(!o){alert('Preencha os dados necessários para calcular o OEE antes de salvar.');return}state.oee=o;renderOeeSaved();persist();closeOee()}
function clearOee(){state.oee=null;renderOeeSaved();persist()}
function renderOeeSaved(){const box=els.oeeSavedSummary;if(!box)return;const o=state.oee;if(!o){box.style.display='none';box.innerHTML='';return}box.style.display='block';box.innerHTML=`<div><strong>OEE salvo: ${oeePct(o.oee)}</strong><span>Disp. ${oeePct(o.availability)} · Perf. ${oeePct(o.performance)} · Qual. ${oeePct(o.quality)}</span></div><button type="button" data-action="clearOee" aria-label="Limpar OEE salvo">×</button>`}
...
function bind(){
  ...
  ['oeeActualQty','oeeScrapQty','oeeReworkQty','oeeReworkAsLoss'].forEach(id=>$(id)?.addEventListener('input',renderOeeCalc));
