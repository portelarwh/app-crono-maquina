...
function recordLap(t){if(!state.running||!state.currentLapStartMs)return;const cause=(t&&t.dataset&&t.dataset.cause)||'Normal';const now=Date.now(),durationMs=now-state.currentLapStartMs;let q=n(els.units,1)||1;if(els.analysisMode.value==='interval'){const d=n(els.defaultLapQty,NaN);q=Number.isFinite(d)&&d>0?d:null;if(els.lapQtyMode.value==='durante'){if(state.pendingLap){els.qtyModalInput.focus();return}state.pendingLap={durationMs,endedAt:now,cause};els.qtyModalInput.value=q||'';els.qtyModal.style.display='flex';els.qtyModalInput.focus();return}}addLap(durationMs,q,now,cause)}
function addLap(durationMs,q,endedAt,causeOverride){const cause=String(causeOverride||'Normal').trim();state.laps.push({id:id(),durationMs:Math.max(0,durationMs),qty:q,obs:(els.lapObs?.value||'').trim(),cause,endedAt});state.currentLapStartMs=endedAt;if(els.lapObs)els.lapObs.value='';render();persist()}
...
function renderControls(){const has=state.laps.length>0;els.btnStart.disabled=state.running;els.btnStop.disabled=!state.running;els.btnLap.disabled=!state.running;document.querySelectorAll('.btn-cause').forEach(b=>{b.disabled=!state.running});[els.btnExport,els.btnPNG,els.btnPDF,els.btnWhatsApp].forEach(b=>{if(b)b.disabled=!has})}
...
function closeQtyModal(ok){if(state.pendingLap&&ok){const q=n(els.qtyModalInput,0);addLap(state.pendingLap.durationMs,q,state.pendingLap.endedAt,state.pendingLap.cause)}state.pendingLap=null;els.qtyModal.style.display='none'}
