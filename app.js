...
function render() {
  ...
  elements.statusDisplay.textContent = state.running ? 'Em execução' : 'Parado';
...
function loadState() {
  ...
  try {
    ...
    const hasMachineName = state.machineName.length > 0;
    state.running = Boolean(saved.running) && saved.startedAt !== null && hasMachineName;
...
function bindEvents() {
  ...
  window.addEventListener('pagehide', saveRunningSnapshot);
}
