...
function render(){renderTimersOnly();renderMode();renderStats();renderHistory();renderCharts();renderEventTimeline();renderParadasCards();renderPrint();renderControls()}
...
function renderOeeCalc(){const b=oeeBase(),actual=oeeVal('oeeActualQty',0),scrap=Math.max(0,oeeVal('oeeScrapQty',0)),rework=Math.max(0,oeeVal('oeeReworkQty',0)),reworkLoss=$('oeeReworkAsLoss')?.checked?rework:0,good=Math.max(0,actual-scrap-reworkLoss),availability=b.plannedMs>0?Math.min(1,b.runMs/b.plannedMs):NaN,performance=b.expectedQty>0?Math.min(1,actual/b.expectedQty):NaN,quality=actual>0?Math.min(1,good/actual):NaN,oee=[availability,performance,quality].every(Number.isFinite)?availability*performance*quality:NaN;oeeSet('oeeTotalTime',oeeTime(b.plannedMs));oeeSet('oeeDowntimeTime',oeeTime(b.downtimeMs));oeeSet('oeeRunTime',oeeTime(b.runMs));oeeSet('oeeConfiguredSpeed',b.configuredRate>0?`${oeeQty(b.configuredRate)} ${b.unitLabel}`:'—');oeeSet('oeeExpectedQty',b.expectedQty>0?`${oeeQty(b.expectedQty)} un`:'—');oeeSet('oeeValue',oeePct(oee));oeeSet('oeeAvailability',oeePct(availability));oeeSet('oeePerformance',oeePct(performance));oeeSet('oeeQuality',oeePct(quality));const diag=$('oeeDiagnosis');if(!diag)return;if(!b.plannedMs){diag.textContent='Registre tempo de medição antes de calcular o OEE.';return}if(!b.configuredRate){diag.textContent='Informe Meta ou Takt Time para estimar a produção esperada e calcular Performance.';return}if(!actual){diag.textContent='Informe a quantidade real produzida. Se ficar menor que a esperada com máquina rodando, a perda aparecerá em Performance.';return}const losses=[{name:'Disponibilidade',value:Number.isFinite(availability)?1-availability:-1},{name:'Performance',value:Number.isFinite(performance)?1-performance:-1},{name:'Qualidade',value:Number.isFinite(quality)?1-quality:-1}].sort((a,b)=>b.value-a.value);const main=losses[0]&&losses[0].value>0.001?losses[0].name:'Sem perda dominante';const perfNote=actual<b.expectedQty?' A produção real ficou abaixo da esperada para o tempo rodando, indicando queda de performance.':'';diag.textContent=`Principal perda: ${main}. OEE calculado como Disponibilidade × Performance × Qualidade.${perfNote}`}
function openOee(){const modal=els.oeeModal;if(!modal)return;const b=oeeBase();$('oeeActualQty').value=b.stats.q?Number(b.stats.q).toFixed(2):'';$('oeeScrapQty').value='0';$('oeeReworkQty').value='0';if($('oeeReworkAsLoss'))$('oeeReworkAsLoss').checked=true;modal.style.display='flex';renderOeeCalc();setTimeout(()=>$('oeeActualQty')?.focus(),50)}
