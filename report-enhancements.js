...
(function(){
  ...
  function data(){
    ...
    return {
      version: window.APP_VERSION || 'v5.1.31',
  ...
  function oeeBar(label, value) {
    ...
    var pct = valid ? Math.max(0, Math.min(100, v * 100)) : 0;
    var color = !valid ? '#aaa' : v >= 0.75 ? '#1e9a44' : v >= 0.60 ? '#e09a00' : '#df1f2d';
    var txt = valid ? pct.toFixed(1) + '%' : '—';
    return '<div style="min-width:0">'
      + '<div style="font-size:7px;font-weight:900;color:#58677d;text-transform:uppercase;letter-spacing:.04em;margin-bottom:3px">'+esc(label)+'</div>'
      + '<div style="height:10px;background:#e2e8f0;border-radius:5px;overflow:hidden">'
      + '<div style="width:'+pct.toFixed(1)+'%;height:100%;background:'+color+';border-radius:5px;min-width:2px"></div>'
      + '</div>'
      + '<div style="font-size:11px;font-weight:900;color:'+color+';margin-top:2px;text-align:right;line-height:1">'+esc(txt)+'</div>'
      + '</div>';
  }
  ...
  function oeeBlock(){
    ...
    return '<section class="panel diagnostic oeePanel '+cls+'">'
      + '<h2>OEE — Disponibilidade · Performance · Qualidade</h2>'
      + '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:6px 0 8px">'
      + oeeBar('OEE', o.oee)
      + oeeBar('Disponib.', o.availability)
      + oeeBar('Performance', o.performance)
      + oeeBar('Qualidade', o.quality)
      + '</div>'
  ...
  function foot(){ return '<footer class="foot"><div><b>Crono Máquina</b>Sistema de cronoanálise e tempo padrão</div><div><b>PULSAR OPS</b></div><div class="sig">Assinatura do Analista</div></footer>'; }
