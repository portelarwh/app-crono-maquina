...
(function(){
  ...
  function injectFields(){
    ...
    block.className = 'extra-config-grid full-width';
    block.innerHTML = `
      ...
      <div class="input-group extra-shift-hours"><label>Horas/turno</label><input id="shiftHours" type="number" value="8" min="0.1" step="0.1" inputmode="decimal"></div>
      <div class="input-group extra-tolerance full-width"><label>Tolerância para tempo padrão (%)</label><input id="tolerancePct" type="number" value="10" min="0" step="0.5" inputmode="decimal"></div>
    `;
  ...
  function normalizeBaseData(base){
    ...
    return {
      version: base.version || window.APP_VERSION || 'v5.1.26', running: !!base.running, totalElapsedMs: Number(base.totalElapsedMs) || 0,
  ...
  function renderImpact(data){ const i = data?.impact || {}; const setCard=(id,val,goodWhen)=>{const el=$(id); if(!el)return; const card=el.parentElement; el.textContent=val; if(card){card.classList.remove('cardGood','cardBad'); if(i.target){card.classList.add(goodWhen?'cardGood':'cardBad');}}}; setCard('valCapacityGap', i.target?`${formatNumber(i.gap,1)} ${i.unitLabel} (${formatNumber(i.gapPct,1)}%)`:'--', (i.gap||0)>=0); setCard('valLossHour', i.target?`${formatNumber(i.lossPerHour,0)} un/h`:'--', (i.lossPerHour||0)<=0); setCard('valLossShift', i.target?`${formatNumber(i.lossPerShift,0)} un/turno`:'--', (i.lossPerShift||0)<=0); }
  ...
  function injectLapCauseSelectors(data){
    ...
    rows.forEach((row, index) => {
      ...
      if(!existing) {
        ...
        const deleteBtn = row.querySelector('[data-action="deleteEvent"]');
        if(deleteBtn) row.insertBefore(wrap, deleteBtn);
  ...
  function initCauseToggle(){
    const wrap = document.getElementById('causeSectionWrap');
    const btn  = document.getElementById('btnToggleCauses');
    if(!wrap || !btn) return;
    const PREF_KEY = 'causesCollapsed';
    function apply(collapsed){
      wrap.classList.toggle('collapsed', collapsed);
      btn.textContent = collapsed ? '▸' : '▾';
      btn.title = collapsed ? 'Mostrar tipos de parada' : 'Ocultar tipos de parada';
