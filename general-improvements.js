...
(function(){
  ...
  function injectStyles(){
    ...
    style.textContent = `
      ...
      @media(max-width:560px){.impact-grid,.advanced-grid,.compare-row{grid-template-columns:1fr}.lap-cause-select-row{width:100%;margin:6px 0 0 0}}
    `;
  ...
  function normalizeBaseData(base){
    ...
    return {
      version: base.version || window.APP_VERSION || 'v2.4.11', running: !!base.running, totalElapsedMs: Number(base.totalElapsedMs) || 0,
  ...
  function renderStudyOptions(){ const base = $('studyBase'), compare = $('studyCompare'); if(!base || !compare) return; const list = getStudies(); const options = list.map(study => `<option value="${study.id}">${escapeHtml(study.savedAt)} — ${escapeHtml(study.name)}</option>`).join(''); base.innerHTML = options || '<option value="">Nenhum estudo salvo</option>'; compare.innerHTML = '<option value="current">Medição atual</option>' + options; }
  ...
  function bindEvents(){
    document.addEventListener('click', event => { if(event.target?.id === 'btnUndoLap') undoLastLap(); if(event.target?.id === 'btnSaveStudy') saveStudy(); if(event.target?.id === 'btnCompareStudy') compareStudies(); setTimeout(updateAll, 80); });
    ...
    document.addEventListener('change', event => { if(event.target && ['lineName','shiftName','productName','shiftHours','tolerancePct'].includes(event.target.id)) persistExtras(); setTimeout(updateAll, 80); });
